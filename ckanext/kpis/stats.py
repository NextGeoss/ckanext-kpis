# -*- coding: utf-8 -*-
"""Tools for calculating portal stats and KPIs."""
...
def get_first_date():
    ...
    if not config_date:
        ...
        s = select([func.min(revision.c.timestamp)], from_obj=[revision])
...
class UsageStats(object):
    """Collection of methods for calculating portal usage statistics."""

    @classmethod
    def get_weeks(cls):
        """Return a list of weeks."""
        today = datetime.datetime.utcnow().date()
        week_start = first_date - datetime.timedelta(
            days=datetime.date.weekday(first_date))
        weeks = []
    ...
    def get_monthly_user_counts(cls, tracking_type):
        """Return a list of user counts by month."""
        months = cls.get_months()
        visits_for_months = cls.get_visit_counts_for_months(
            tracking_type, months)
        return visits_for_months

    @classmethod
    def get_dataset_counts(cls, set_type):
        """Return a list of dataset counts by week."""
        weeks = cls.get_weeks()
    ...
    def get_visit_counts_for_months(cls, tracking_type, months):
        """Return a list of user counts and percentage completion by month."""
        ...
        for month in months:
            ...
            visit_counts_for_months.append(
                (month[0].strftime(DATE_FORMAT), count, percent)
            )
        return visit_counts_for_months

    @classmethod
    def get_dataset_counts_for_weeks(cls, set_type, weeks):
        """Return a list of dataset counts and % completion by week."""
        ...
        for week in weeks:
            ...
            dataset_counts_for_weeks.append(
                (week[0].strftime(DATE_FORMAT), count, percent)
            )
        return dataset_counts_for_weeks

    @classmethod
    def get_datasets_for_week(cls, set_type, week):
        """Return the change in the number of datasets for a given week."""
        ...
        s = select(
            [func.count(package_revision.c.id)],
            from_obj=[package_revision.join(revision)]).\
            where(
                and_(
                    package_revision.c.type == set_type,
                    package_revision.c.state == model.State.DELETED,
                    revision.c.timestamp >= week[0],
                    revision.c.timestamp <= week[1]
                )
        )
    ...
    def get_visits_for_month(cls, tracking_type, month):
        ...
        if tracking_type == 'all':
            ...
            tracking_query = and_(
                tracking_raw.c.tracking_type == tracking_type,
                tracking_raw.c.access_timestamp >= month[0],
                tracking_raw.c.access_timestamp <= month[1]
            )

        s = select(
            [func.count(distinct(tracking_raw.c.user_key))],
            from_obj=[tracking_raw]).where(tracking_query)
